...
class Arena:
    ...
    def lineup(matches: List[Tuple[Player, Player]], both_side=False, show_results=False) -> List[tuple]:
        result = [Avalam(p0, p1).play_full(show_results) for p0, p1 in matches]
        if both_side:
            result += [Avalam(p1, p0).play_full(show_results) for p0, p1 in matches]

        return result

    @staticmethod
    def versus(match: Tuple[Player, Player], both_side=False, show_results=False) -> List[tuple]:
        result = [Avalam(match[0], match[1]).play_full(show_results)]
        if both_side:
            result = [result, Avalam(match[1], match[0]).play_full(show_results)]
    ...
    def round_robin(players: List[Player], show_results=False) -> Dict[str, tuple]:
        ...
        for p0, p1 in matches:
            res = Avalam(p0, p1).play_full(show_results)
